...
//#define SPEED_RUN
//#define GOD_MODE
// #define DEFAULT_WEAPON_MACHINE_GUN
#define sniperHP gSoldier0State.w
#define DEFAULT_SNIPER_HP 5.0
// TODO add turretHP, bossTurretHP, BossCoreHP
...
const float SNIPER_FIRE_RATE		= 64.0; // 160
const float TURRET_FIRE_RATE		= 50.0;
...
const float BRIDGE_0_START_TILE		= 25.0;
...
void UpdateSpawner( float screenWidth )
{
    ...
    vec4 newSoldierState 	= vec4( 0.0, 0.0, 0.0, gSoldier0State.w/* reserve for sniper HP */ );
...
void PlayerBulletSniperTest( inout vec4 playerBullet, inout vec4 sniper )
{
	if ( playerBullet.x > 0.0 && Collide( playerBullet.xy, BULLET_SIZE, sniper.xy, SNIPER_SIZE ) )
    {
        gHit		  	= vec4( playerBullet.xy, 0.0, 0.0 );
        playerBullet.x 	= 0.0;
        // sniperHP <=> gSoldier0State.w
        
        --sniperHP; // decrease HP
        if( sniperHP <= 0.0) {
            // kill sniper
...
void PlayerHit( vec4 playerHitBox )
{
    ...
    if ( gGameState.x == GAME_STATE_LEVEL && gGameState.y > UI_GAME_START_TIME )
    {
        ...
        gPlayerDir		= vec4( 1.0, 0.0, 0.0, PLAYER_IMMORTALITY_LEN );
          ...
          #ifdef DEFAULT_WEAPON_MACHINE_GUN
...
void UpdateSoldier( inout vec4 soldier, inout vec4 soldierState, vec4 playerHitBox, float screenWidth, float screenHeight )
{
    ...
    if ( soldier.x > 0.0 && soldier.y < WATER_HEIGHT )   
    {
        ...
        // fun death :)? if soldiers falling in the water, player being hit
        //PlayerHit( playerHitBox );
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    else if ( gPlayerState.x == STATE_PRONE )
    {
        ...
        else if ( keyJump ) // keyJump && (keyDown && !keyRight && !keyLeft)
        {
            gPlayerState.x = STATE_FALL;
    ...
    if(gPlayerWeapon.x == WEAPON_RIFLE) {
    ...
    } else { // if gPlayerWeapon.x == WEAPON_MACHINE_GUN
      if ( keyShoot && gPlayerWeapon.y > gPlayerWeapon.z && playerBulletNum < gPlayerWeapon.w && gPlayerState.x != STATE_UNDER_WATER )
      {
          gPlayerWeapon.y = 0.0;
          
          vec2 normPlayerDir = normalize(gPlayerDir.xy);
          // rotate vector to 90 degree left: newX = -y, newY = x
          const float bullet_spread = 0.1;
          vec2 dr = bullet_spread * vec2(normPlayerDir.y * (-1.0), normPlayerDir.x * 1.0);
          vec2 dir1 = gPlayerDir.xy + dr;
          vec2 dir2 = gPlayerDir.xy - dr;
          
          if ( gPlayerBullet0.x <= 0.0 && gPlayerBullet1.x <= 0.0 )
          {
            gPlayerBullet0.xy = playerWeapon;
              gPlayerBullet0.zw = normalize( dir1 );
            gPlayerBullet1.xy = playerWeapon;
              gPlayerBullet1.zw = normalize( dir2 );
          }
          else if ( gPlayerBullet1.x <= 0.0 && gPlayerBullet2.x <= 0.0 )
          {
            gPlayerBullet1.xy = playerWeapon;
              gPlayerBullet1.zw = normalize( dir1 );
            gPlayerBullet2.xy = playerWeapon;
              gPlayerBullet2.zw = normalize( dir2 );
          }
          else if ( gPlayerBullet2.x <= 0.0 && gPlayerBullet3.x <= 0.0 )
          {
            gPlayerBullet2.xy = playerWeapon;
              gPlayerBullet2.zw = normalize( dir1 );
            gPlayerBullet3.xy = playerWeapon;
              gPlayerBullet3.zw = normalize( dir2 );
          }     
          else if ( gPlayerBullet3.x <= 0.0 && gPlayerBullet4.x <= 0.0 )
          {
            gPlayerBullet3.xy = playerWeapon;
              gPlayerBullet3.zw = normalize( dir1 );
            gPlayerBullet4.xy = playerWeapon;
              gPlayerBullet4.zw = normalize( dir2 );
          }
          else if ( gPlayerBullet4.x <= 0.0 && gPlayerBullet5.x <= 0.0 )
          {
            gPlayerBullet4.xy = playerWeapon;
              gPlayerBullet4.zw = normalize( dir1 );
            gPlayerBullet5.xy = playerWeapon;
              gPlayerBullet5.zw = normalize( dir2 );
          }  
          else if ( gPlayerBullet5.x <= 0.0 )
          {
            gPlayerBullet5.xy = playerWeapon;
