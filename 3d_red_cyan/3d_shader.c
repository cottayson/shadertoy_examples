...
vec2 Scene(in vec3 rO, in vec3 rD, in vec2 fragCoord)
{
	...
	{
		// if (t > 12.0) break;
        if (t > 12.0) break; // max distance of calculated object
        p = rO + t*rD;
...
vec3 PostEffects(vec3 rgb, vec2 xy)
{
	...
	// Then...
	#define CONTRAST 0.95// 1.08
	#define SATURATION 0.8 // 1.5
	#define BRIGHTNESS 2.0 // 1.5
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	#ifdef STEREO
	cameraPos += /*.008*/-0.05*cu*isRed; // move camera to the LEFT because red is left eye
	#endif
