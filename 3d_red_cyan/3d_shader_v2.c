...
float Map(vec3 pos) 
{
    ...
    vec3 s1pos = vec3(-3.0, 0.0, -1.5);
    vec3 s2pos = vec3(0.0, 2.0, 1.0);
    float s1 = length(p.xyz - s1pos) - 1.0; // s1 <=> sphere1
    float s2 = length(p.xyz - s2pos) - 2.0;
	return min(s1, s2);
}
...
vec2 Scene(in vec3 rO, in vec3 rD, in vec2 fragCoord)
{
	...
	{
		// if (t > 12.0) break;
        if (t > 120.0) break; // max distance of calculated object
        p = rO + t*rD;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 cameraPos = vec3(-10.0, 5.0, 0.0);
    ...
    float d_boundary = 8.0;
    if (ret.x > d_boundary && ret.x < d_boundary + 0.05) {
    	col += vec3(0.0, 1.0, 0.0) * 100.0;
    }
