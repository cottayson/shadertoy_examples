...
#define STEREO
#define STEREO_WIDTH -0.5

vec3 sunDir  = normalize( vec3(  0.35, 0.1,  0.3 ) );
const vec3 sunColour = vec3(1.0, .95, .5);

// ------------ COLORS ------------
#define AMBIENT_COLOR vec3(0.4, 0.4, 0.6)
#define REFLECTION_COEF 1.0
#define CONTRAST 1.0// 1.08
#define SATURATION 1.0 // 1.5
#define BRIGHTNESS 1.0 // 1.5
...
// ------------ GEOMETRY ------------
#define ITERATIONS 10 // 9
#define SCALE 2.85// 2.8
#define MINRAD2 0.25 // 0.25
...
float Map(vec3 pos) 
{
    ...
    vec3 s1pos = vec3(-3.0, 1.0, -1.5);
    vec3 s2pos = vec3(-3.0, 1.0, -0.2);
    vec3 s3pos = vec3(-8.0, 4.0, -1.0);
    float s1 = length(p.xyz - s1pos) - 1.0; // s1 <=> sphere1
    float s2 = length(p.xyz - s2pos) - 0.3;
    float s3 = length(p.xyz - s3pos) - 0.3;
...
vec2 Scene(in vec3 rO, in vec3 rD, in vec2 fragCoord)
{
	...
	{
		// if (t > 12.0) break;
        if (t > MAX_DISTANCE) break; // max distance of calculated object
        p = rO + t*rD;
       
		float h = Map(p);
        
		if(h < MIN_STEP) // h < 0.0005
		{
            dist = vec2(oldT, t);
            hit = true;
            break;
        }
       	//glow += clamp(0.05-h, 0.0, .4);
        oldT = t;
      	//t +=  h + t*0.001;
        t +=  (h + 0.0 * t);
 	}
...
vec3 CameraPath( float t )
{
    ...
    vec3 p = vec3(10.0 - 0.1 * t, 0.5 * sin(0.1 * t), -0.3*sin(0.2 * t));
	return p;
} 
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec2 uv = FOV * (-1.0 + 2.0 * xy) * vec2(iResolution.x/iResolution.y, 1.0);
    ...
    vec3 cameraPos = vec3(-10.0, 5.0 + sin(0.5 * iTime) * 1.0, 0.0);
	...
	#ifdef STEREO
	cameraPos += /*.008*/STEREO_WIDTH*cu*isRed; // move camera to the LEFT because red is left eye
	#endif
    ...
    if (ret.x < 900.0)
    {
        ...
        float shaSpot = Shadow(p, spot) * 10.0;
        float shaSun = Shadow(p, sunDir) * 10.0;
       ...
       vec3 ref = reflect(dir, nor) * REFLECTION_COEF;
       col += pow(max(dot(spot,  ref), 0.0), 2.0/*10.0*/) * 2.0 * shaSpot * bri;
       col += pow(max(dot(sunDir, ref), 0.0), 2.0/*10.0*/) * 2.0 * shaSun  * bri;
    } else {
    	
    }
    float d_boundary = 0.0;
    float d1 = mod(p.x, 0.25);
    float opacity = 0.9;
    	col += mix(vec3(0.0), vec3(1.0, 1.0, 1.0), opacity) * smoothstep(0.0, 0.05, d1) * smoothstep(0.1, 0.05, d1);
    //}
    
    col += AMBIENT_COLOR * 0.1;
    
    col = mix(sky*10.0, col, min(exp(-ret.x*0.2+1.5), 1.0));
	...
	#ifdef STEREO
    //float lambda = -0.1;
    //float newRed = isRed + (1.0 - isRed) * lambda;
